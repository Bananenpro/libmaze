...
bool Maze::get(std::size_t pos, GridType gridType) const { return mData.at(getIndexOf(pos, gridType)); }

bool Maze::get(const Point &point, GridType gridType) const { return mData.at(getIndexOf(point, gridType)); }
...
std::size_t Maze::size(GridType gridType) const { return width(gridType) * height(gridType); }

std::size_t Maze::entrancePos(GridType gridType) const { return mEntrancePos; }

std::size_t Maze::exitPos(GridType gridType) const { return mExitPos; }

// ********** Setters **********

void Maze::set(std::size_t pos, GridType gridType, bool value) { mData.at(getIndexOf(pos, gridType)) = value; }

void Maze::set(const Point &point, GridType gridType, bool value) { mData.at(getIndexOf(point, gridType)) = value; }
...
void Maze::save(IExporter &exporter, const std::string &path) const { exporter.save(*this, path); }

Maze Maze::load(ILoader &loader, const std::string &path) { return loader.load(path); }
...
std::size_t Maze::getIndexOfWallOfCell(const Point &point, Direction direction) const
{
    ...
    if (wallPoint.x == 0 || wallPoint.y == 0 || wallPoint.x == width(GridType::ALL) - 1 ||
        wallPoint.y == height(GridType::ALL) - 1) {
